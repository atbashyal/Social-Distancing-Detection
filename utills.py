...
'''
Contains functions to calculate bottom center for all bounding boxes and transform prespective for all points, calculate distance between humans, calculate width and height scale ratio for bird eye view, and calculates number of humans at risk, low risk, no risk according to closeness.
'''
...
# Function calculates distance between two points(humans). distance_w, distance_h represents number of pixels in 180cm length horizontally and vertically. We calculate horizontal and vertical distance in pixels for two points and get ratio in terms of 180 cm distance using distance_w, distance_h. Then we calculate how much cm distance is horizontally and vertically and then using pythagoras we calculate distance between points in terms of cm. 
